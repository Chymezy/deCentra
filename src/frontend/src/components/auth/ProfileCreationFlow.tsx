...
export function ProfileCreationFlow({
  ...
  const validateUsername = useCallback(
    async (username: string) => {
      ...
      if (
        ...
        setErrors((prev) => ({
          ...prev,
          username: 'Username cannot start or end with underscore or hyphen',
        }));
        return false;
      }
      ...
      if (username && /[_-]{2,}/.test(username)) {
        setErrors((prev) => ({
          ...prev,
          username: 'Username cannot have consecutive special characters',
        }));
        return false;
      }
  ...
  const validateBio = useCallback((bio: string) => {
    ...
    if (
      ...
      setErrors((prev) => ({
        ...prev,
        bio: 'Bio contains potentially harmful content',
      }));
      return false;
    }
  ...
  const validateAvatar = useCallback((avatar: string) => {
    if (avatar.length > 200) {
      setErrors((prev) => ({
        ...prev,
        avatar: 'Avatar must be less than 200 characters',
      }));
      return false;
    }
    ...
    if (
      ...
      try {
        ...
        if (!avatar.startsWith('https://')) {
          setErrors((prev) => ({
            ...prev,
            avatar: 'Avatar URL must use HTTPS',
          }));
          return false;
        }
        ...
        if (!isAllowed) {
          setErrors((prev) => ({
            ...prev,
            avatar: 'Avatar URL must be from a trusted domain',
          }));
          return false;
        }
      } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (errorMessage.includes('Username already taken')) {
        ...
        setErrors({
          general: 'You already have a profile. Try refreshing the page.',
        });
        ...
        <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-slate-700/50 p-8">
          ...
          <div className="text-center mt-6">
            <p className="text-slate-500 text-xs">
              By creating a profile, you agree to deCentra&apos;s community
              guidelines
            </p>
          </div>
        </div>
