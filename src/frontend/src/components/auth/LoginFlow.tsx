...
const LoginFlow = React.forwardRef<HTMLDivElement, LoginFlowProps>(
  (
    ...
    const [selectedPrivacyMode, setSelectedPrivacyMode] =
      React.useState<PrivacyMode>('normal');
      ...
      return (
        <Card ref={ref} className={cn('w-full max-w-md', className)}>
          <CardHeader className="text-center">
            <CardTitle className="text-privacy-success">
              Welcome Back!
            </CardTitle>
            <CardDescription>
          ...
          <CardContent className="space-y-4">
            <div className="text-center">
              <p className="text-sm text-privacy-text-muted mb-2">
                Privacy Mode:{' '}
                <span className="capitalize font-medium text-privacy-accent">
    ...
    return (
      <Card
        ...
        className={cn(
          'w-full max-w-lg',
          mode === 'modal' && 'border-0 shadow-none',
          className
        )}
      >
        <CardHeader className="text-center">
          ...
          <CardDescription>
            {description ||
              'Choose your privacy level and sign in with Internet Identity'}
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          {showPrivacyModeSelection && (
            <div className="space-y-3">
              <h3 className="text-sm font-medium text-privacy-text">
                Select Privacy Mode
              </h3>
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-privacy-text-muted text-center leading-relaxed">
              ...
              Policy. Your data remains under your control and fully
              decentralized.
            </p>
          </div>
        </CardContent>
      ...
      <div className="flex items-start space-x-3">
        <div
          className={cn(
            ...
            selected
              ? 'bg-privacy-accent text-privacy-dark'
              : 'bg-privacy-muted text-privacy-text-muted'
          )}
        >
          {mode.icon}
        </div>
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-privacy-text-muted mt-1">
            {mode.description}
          </p>
          <ul className="mt-2 space-y-1">
            {mode.features.map((feature, index) => (
              <li
                key={index}
                className="text-xs text-privacy-text-muted flex items-center"
              >
                <svg
                  className="w-3 h-3 mr-1 text-privacy-success"
                  fill="currentColor"
                  viewBox="0 0 20 20"
                >
                  <path
